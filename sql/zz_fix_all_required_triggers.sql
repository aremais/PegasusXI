-- One-time / repair: installs all triggers required by map server db::checkTriggers().
-- Safe to re-run: uses DROP TRIGGER IF EXISTS before CREATE.
-- Sources: sql/triggers.sql, synth_recipes.sql (trigger only), synergy_recipes.sql (trigger only).
