...
void MapSocket::receive()
{
    ...
    socket_.async_receive_from(
        asio::buffer(buffer_), remoteEndpoint_, [this](const std::error_code& ec, std::size_t bytes_recvd)
        {
            ...
            const auto buffer = std::span<uint8>(buffer_.data(), bytes_recvd);

            DebugPackets("Received %zu bytes from %s",
...
void MapSocket::send(const IPP& ipp, ByteSpan buffer)
{
    ...
    DebugPackets("Sending %zu bytes to %s",
                 static_cast<size_t>(buffer.size()),
                 ipp.toString().c_str());

    // Like with the ip from receive(), ASIO is expecting us to be handling it
