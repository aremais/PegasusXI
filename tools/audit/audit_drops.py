...
def _load_dropid_to_zoneids(sql_dir: Path) -> dict[int, list[int]]:
    """Build dropId -> [zoneid, ...] from mob_groups so FFXIDB lookups know
    which (zone, mob) tuple to try.
    """
    ...
    for row in parse_inserts(groups_path, "mob_groups"):
        ...
        if not dropid:
            continue
        zid = d["zoneid"]
        if zid not in out[dropid]:
            out[dropid].append(zid)
    return dict(out)
...
DROP_REF_FIELDS = [
    "ffxidb_avg_pct", "ffxidb_th0_pct", "ffxidb_th1_pct",
    "ffxidb_th2_pct", "ffxidb_th3_pct",
    "ffxidb_kills", "ffxidb_confidence", "ffxidb_url",
    "bgwiki_drop_pct", "bgwiki_kills", "bgwiki_confidence", "bgwiki_url",
]


def _attach_drop_refs(
    csv_rows: list[list[Any]],
    csv_header: list[str],
    *,
    ffxidb_client,
    bgwiki_client,
    dropid_to_zoneids: dict[int, list[int]],
    names_filter: set[str] | None,
) -> list[list[Any]]:
    """Append per-row reference columns to ``csv_rows`` using FFXIDB + BG Wiki.

    FFXIDB lookups are cached per (lsb_zoneid, mob_name) and per (mob_name,
    item_name); we walk every (dropid, item) row and try the first known
    zoneid for that dropid. BG Wiki drop-rate templates are scanned on the
    mob's wiki page.
    """
    from .refs.aggregate import lookup_ffxidb_drops
    from .refs.confidence import kills_to_bucket

    dropid_idx = csv_header.index("dropId")
    item_idx = csv_header.index("itemId")
    iname_idx = csv_header.index("item_name")
    mobs_idx = csv_header.index("mob_names")

    ffxidb_cache: dict[tuple[int, str], dict] = {}
    bgwiki_cache: dict[str, dict[str, tuple[int, int]]] = {}

    out_rows: list[list[Any]] = []
    for row in csv_rows:
        dropid = int(row[dropid_idx])
        iname = (row[iname_idx] or "").strip()
        mobs = [m for m in (row[mobs_idx] or "").split(";") if m]
        mob_name = mobs[0] if mobs else ""

        extra = {f: "" for f in DROP_REF_FIELDS}
        if mob_name and (names_filter is None or mob_name in names_filter):
            zoneids = dropid_to_zoneids.get(dropid, [])
            ffxidb_hit = None
            if ffxidb_client is not None:
                for zid in zoneids:
                    key = (zid, mob_name)
                    drops = ffxidb_cache.get(key)
                    if drops is None:
                        drops = lookup_ffxidb_drops(mob_name, lsb_zoneid=zid, client=ffxidb_client)
                        ffxidb_cache[key] = drops
                    if iname and iname.lower() in drops:
                        ffxidb_hit = drops[iname.lower()]
                        break
            if ffxidb_hit is not None:
                extra["ffxidb_avg_pct"] = ffxidb_hit.avg_pct if ffxidb_hit.avg_pct is not None else ""
                extra["ffxidb_th0_pct"] = ffxidb_hit.th0_pct if ffxidb_hit.th0_pct is not None else ""
                extra["ffxidb_th1_pct"] = ffxidb_hit.th1_pct if ffxidb_hit.th1_pct is not None else ""
                extra["ffxidb_th2_pct"] = ffxidb_hit.th2_pct if ffxidb_hit.th2_pct is not None else ""
                extra["ffxidb_th3_pct"] = ffxidb_hit.th3_pct if ffxidb_hit.th3_pct is not None else ""
                extra["ffxidb_kills"] = ffxidb_hit.kills_total if ffxidb_hit.kills_total is not None else ""
                extra["ffxidb_confidence"] = ffxidb_hit.confidence
                extra["ffxidb_url"] = ffxidb_hit.source_url

            if bgwiki_client is not None and iname:
                wiki_title = mob_name.replace("_", " ")
                drops_map = bgwiki_cache.get(wiki_title)
                if drops_map is None:
                    page = bgwiki_client.get_page(wiki_title)
                    drops_map = {}
                    if not page.missing:
                        for rate in page.drop_rates():
                            drops_map["__page_total__"] = (rate.drops, rate.kills)
                            drops_map.setdefault(iname.lower(), (rate.drops, rate.kills))
                    bgwiki_cache[wiki_title] = drops_map
                if iname.lower() in drops_map:
                    bdrops, bkills = drops_map[iname.lower()]
                    extra["bgwiki_drops"] = bdrops
                    extra["bgwiki_kills"] = bkills
                    extra["bgwiki_drop_pct"] = round(100.0 * bdrops / bkills, 2) if bkills else ""
                    extra["bgwiki_confidence"] = kills_to_bucket(bkills)
                    page_url_holder = bgwiki_client.get_page(wiki_title)
                    extra["bgwiki_url"] = page_url_holder.url

        out_rows.append(row + [extra[f] for f in DROP_REF_FIELDS])
    return out_rows
...
def run(
    ...
    *,
    ...
    dropid_zones = _load_dropid_to_zoneids(local_sql_dir)
    if not dropid_zones:
        dropid_zones = _load_dropid_to_zoneids(upstream_sql_dir)
    ...
    ref_present = with_refs and (ffxidb_client is not None or bgwiki_client is not None)
    if ref_present:
        csv_rows = _attach_drop_refs(
            csv_rows, csv_header,
            ffxidb_client=ffxidb_client,
            bgwiki_client=bgwiki_client,
            dropid_to_zoneids=dropid_zones,
            names_filter=names_filter,
        )
        csv_header = csv_header + DROP_REF_FIELDS
    ...
    if ref_present:
        md_summary_header += ["ffxidb_th0_pct", "ffxidb_kills", "ffxidb_confidence"]
    ...
    for r in csv_rows[:500]:  # cap Markdown preview
        if ref_present:
            (
                ...
                avg_pct, th0_pct, th1_pct, th2_pct, th3_pct,
                ffxidb_kills, ffxidb_conf, ffxidb_url,
                bgwiki_pct, bgwiki_kills, bgwiki_conf, bgwiki_url,
            ) = r
            md_rows.append([
                dropid, gid, iid, iname, mnames, status, changes,
                f"{l_gr}/{l_ir}", f"{u_gr}/{u_ir}",
                th0_pct, ffxidb_kills, ffxidb_conf,
            ])
        else:
            (
                dropid, gid, iid, iname, mnames, status, changes,
...
def main(argv: list[str] | None = None) -> int:
    ...
    p.add_argument("--with-refs", action="store_true",
                   help="Enrich rows with FFXIDB / BG Wiki drop-rate lookups.")
    p.add_argument("--zone-map", type=Path, default=None,
                   help="JSON file mapping LSB zoneid -> FFXIDB zoneid; required for FFXIDB.")
    p.add_argument("--names-file", type=Path, default=None,
                   help="Restrict reference lookups to mob names in this file.")
    p.add_argument("--ffxidb-todo", type=Path, default=None,
                   help="CSV path to record FFXIDB lookups that failed (missing zone map, etc.).")
    ...
    if args.with_refs:
        ...
        from .refs.ffxidb import FFXIDBClient, ZoneMap
        zm = ZoneMap.from_json(args.zone_map) if args.zone_map else ZoneMap()
        ffxidb_client = FFXIDBClient(
            zone_map=zm,
            allow_fetch=not args.no_fetch,
            todo_path=args.ffxidb_todo,
        )
